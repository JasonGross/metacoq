...
Ltac handle_typing_by_tc _ :=
  lazymatch goal with
  | [ |- @typing ?cf (?Σ, ?ϕ) [] ?t ?T ]
    => notypeclasses refine (@typing_quoted_term_of_general_empty_ctx _ _ _ _ T t ϕ _ cf Σ _ _ _ _);
       [ typeclasses eauto | .. ]
  | [ |- ?G ]
...
Section __.
  ...
  #[local] Instance ground_quotable_well_typed_bool (Σ:=use [bool]) : ground_quotable_well_typed Σ bool
    := ltac:(intros [] Hwf; cbv; handle_typing_by_safechecker cf Σ; apply extends_refl).
(*
  #[local] Instance ground_quotable_well_typed_eq {A} {qA : quotation_of A} {quoteA : ground_quotable A} {x y : A}
    (Σ:=use [bool]) : ground_quotable_well_typed Σ bool
    := ltac:(intros [] Hwf; cbv; handle_typing_by_safechecker cf Σ; apply extends_refl).
...
*)
  ...
  #[local] Instance ground_quotable_well_typed_eq_true (Σ:=use [eq_true]) {b} : ground_quotable_well_typed Σ (eq_true b)
    := ltac:(intros [] Hwf; cbv; handle_typing_by_safechecker cf Σ; apply extends_refl).
  (*
  #[local] Instance ground_quotable_well_typed_BoolSpec {P Q : Prop} (Σ:=use [eq_true]) {b} : ground_quotable_well_typed Σ (eq_true b)
    := ltac:(intros [] Hwf; cbv; handle_typing_by_safechecker cf Σ; apply extends_refl).
   *)
  Definition unfold_quote_nat_S {n} : Init.quote_nat (S n) = tApp _ (Init.quote_nat n)
    := eq_refl.
  Definition unfold_quote_ground_nat_S {n : nat} : quote_ground (S n) = tApp _ (quote_ground n)
    := eq_refl.
  #[local] Instance ground_quotable_well_typed_nat (Σ:=use [nat]) : ground_quotable_well_typed Σ nat.
  Proof using Type.
    intros n; induction n as [|n IH]; intro Hwf; rewrite ?unfold_quote_ground_nat_S.
    { cbv; handle_typing_by_safechecker cf Σ; apply extends_refl. }
    { prove_ground_quotable_well_typed cf Σ; apply extends_refl. }
  Qed.
(*
