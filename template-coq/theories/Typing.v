...
Goal forall Σ Γ t u, @red1 Σ Γ t u -> Type.
  intros.
  destruct X eqn:equ.
  6: {  Set Nested Proofs Allowed.
Lemma red1_rect :
  forall (Σ : global_env) (P : forall Σ Γ t u, @red1 Σ Γ t u -> Type),
       (forall (Γ : context) (na : aname) (t b a : term) (l : list term),
           P Σ Γ (tApp (tLambda na t b) (a :: l)) (mkApps (b {0 := a}) l)
             (red_beta _ _ _ _ _ _ _)) ->
       (forall (Γ : context) (na : aname) (b t b' : term),
           P Σ Γ (tLetIn na b t b') (b' {0 := b})
             (red_zeta _ _ _ _ _ _)) ->

       (forall (Γ : context) (i : nat) (body : term)
               (e : option_map decl_body (nth_error Γ i) = Some (Some body)),
           P Σ Γ (tRel i) ((lift0 (S i)) body)
             (red_rel _ _ _ _ e)) ->
       ...
       (forall (Γ : context) (ci : case_info) mdecl idecl cdecl (c : nat) (u : Instance.t) (args : list term)
               ...
               (e : nth_error brs c = Some br)
               (d : declared_constructor Σ (ci.(ci_ind), c) mdecl idecl cdecl)
               (bctx := case_branch_context ci.(ci_ind) mdecl cdecl p br)
               (e0 : #|args| = (ci.(ci_npar) + context_assumptions bctx)%nat),
           P Σ Γ (tCase ci p (mkApps (tConstruct ci.(ci_ind) c u) args) brs)
             (iota_red ci.(ci_npar) args bctx br)
             (red_iota _ _ _ _ _ _ _ _ _ _ _ _ e d e0)) ->

       (forall (Γ : context) (mfix : mfixpoint term) (idx : nat) (args : list term) (narg : nat) (fn : term)
               (e : unfold_fix mfix idx = Some (narg, fn))
               (e0 : is_constructor narg args = true),
           P Σ Γ (tApp (tFix mfix idx) args) (mkApps fn args)
             (red_fix _ _ _ _ _ _ _ e e0)) ->
       ...
       (forall (Γ : context) (ip : case_info) (p : predicate term) (mfix : mfixpoint term) (idx : nat)
               ...
               (e : unfold_cofix mfix idx = Some (narg, fn)),
           P Σ Γ (tCase ip p (mkApps (tCoFix mfix idx) args) brs) (tCase ip p (mkApps fn args) brs)
             (red_cofix_case _ _ _ _ _ _ _ _ _ _ e)) ->

       (forall (Γ : context) (p : projection) (mfix : mfixpoint term) (idx : nat) (args : list term)
          (narg : nat) (fn : term),
        unfold_cofix mfix idx = Some (narg, fn) -> P Σ Γ (tProj p (mkApps (tCoFix mfix idx) args)) (tProj p (mkApps fn args))) ->

       (forall (Γ : context) c (decl : constant_body) (body : term),
        declared_constant Σ c decl ->
        forall u : Instance.t, cst_body decl = Some body -> P Σ Γ (tConst c u) (subst_instance u body)) ->

       (forall (Γ : context) p (args : list term) (u : Instance.t) (arg : term),
           nth_error args (p.(proj_npars) + p.(proj_arg)) = Some arg ->
           P Σ Γ (tProj p (mkApps (tConstruct p.(proj_ind) 0 u) args)) arg) ->

       (forall (Γ : context) (na : aname) (M M' N : term),
        red1 Σ Γ M M' -> P Σ Γ M M' -> P Σ Γ (tLambda na M N) (tLambda na M' N)) ->

       (forall (Γ : context) (na : aname) (M M' N : term),
        red1 Σ (Γ,, vass na N) M M' -> P Σ (Γ,, vass na N) M M' -> P Σ Γ (tLambda na N M) (tLambda na N M')) ->

       (forall (Γ : context) (na : aname) (b t b' r : term),
        red1 Σ Γ b r -> P Σ Γ b r -> P Σ Γ (tLetIn na b t b') (tLetIn na r t b')) ->

       (forall (Γ : context) (na : aname) (b t b' r : term),
        red1 Σ Γ t r -> P Σ Γ t r -> P Σ Γ (tLetIn na b t b') (tLetIn na b r b')) ->

       (forall (Γ : context) (na : aname) (b t b' r : term),
        red1 Σ (Γ,, vdef na b t) b' r -> P Σ (Γ,, vdef na b t) b' r -> P Σ Γ (tLetIn na b t b') (tLetIn na b t r)) ->

       (forall (Γ : context) (ind : case_info) params params' puinst pcontext preturn c brs,
           OnOne2 (Trel_conj (red1 Σ Γ) (P Σ Γ)) params params' ->
           P Σ Γ (tCase ind (mk_predicate puinst params pcontext preturn) c brs)
               (tCase ind (mk_predicate puinst params' pcontext preturn) c brs)) ->
       ...
       (forall (Γ : context) (ci : case_info)
          ...
          let p := (mk_predicate puinst params pcontext preturn) in
           ...
           P Σ (Γ ,,, case_predicate_context ci.(ci_ind) mdecl idecl p) preturn preturn' ->
           P Σ Γ (tCase ci p c brs)
       ...
       (forall (Γ : context) (ind : case_info) (p : predicate term) (c c' : term) (brs : list (branch term)),
        red1 Σ Γ c c' -> P Σ Γ c c' -> P Σ Γ (tCase ind p c brs) (tCase ind p c' brs)) ->

       (forall (Γ : context) ind mdecl idecl (isdecl : declared_inductive Σ ind.(ci_ind) mdecl idecl) p c brs brs',
          OnOne2All (fun brctx br br' =>
            on_Trel_eq (Trel_conj (red1 Σ (Γ ,,, brctx)) (P Σ (Γ ,,, brctx))) bbody bcontext br br')
              (case_branches_contexts ind.(ci_ind) mdecl idecl p brs) brs brs' ->
          P Σ Γ (tCase ind p c brs) (tCase ind p c brs')) ->

       (forall (Γ : context) (p : projection) (c c' : term), red1 Σ Γ c c' -> P Σ Γ c c' -> P Σ Γ (tProj p c) (tProj p c')) ->

       (forall (Γ : context) (M1 N1 : term) (M2 : list term), red1 Σ Γ M1 N1 -> P Σ Γ M1 N1 -> P Σ Γ (tApp M1 M2) (mkApps N1 M2)) ->

       (forall (Γ : context) (M2 N2 : list term) (M1 : term), OnOne2 (fun x y => red1 Σ Γ x y * P Σ Γ x y)%type M2 N2 -> P Σ Γ (tApp M1 M2) (tApp M1 N2)) ->

       (forall (Γ : context) (na : aname) (M1 M2 N1 : term),
        red1 Σ Γ M1 N1 -> P Σ Γ M1 N1 -> P Σ Γ (tProd na M1 M2) (tProd na N1 M2)) ->

       (forall (Γ : context) (na : aname) (M2 N2 M1 : term),
        red1 Σ (Γ,, vass na M1) M2 N2 -> P Σ (Γ,, vass na M1) M2 N2 -> P Σ Γ (tProd na M1 M2) (tProd na M1 N2)) ->

       (forall (Γ : context) (ev : nat) (l l' : list term), OnOne2 (fun x y => red1 Σ Γ x y * P Σ Γ x y) l l' -> P Σ Γ (tEvar ev l) (tEvar ev l')) ->

       (forall (Γ : context) (M1 : term) (k : cast_kind) (M2 N1 : term),
        red1 Σ Γ M1 N1 -> P Σ Γ M1 N1 -> P Σ Γ (tCast M1 k M2) (tCast N1 k M2)) ->

       (forall (Γ : context) (M2 : term) (k : cast_kind) (N2 M1 : term),
        red1 Σ Γ M2 N2 -> P Σ Γ M2 N2 -> P Σ Γ (tCast M1 k M2) (tCast M1 k N2)) ->

       (forall (Γ : context) (M1 : term) (k : cast_kind) (M2 : term),
           P Σ Γ (tCast M1 k M2) M1) ->

       (forall (Γ : context) (mfix0 mfix1 : list (def term)) (idx : nat),
          OnOne2 (on_Trel_eq (Trel_conj (red1 Σ Γ) (P Σ Γ)) dtype (fun x => (dname x, dbody x, rarg x))) mfix0 mfix1 ->
          P Σ Γ (tFix mfix0 idx) (tFix mfix1 idx)) ->

       (forall (Γ : context) (mfix0 mfix1 : list (def term)) (idx : nat),
          OnOne2 (on_Trel_eq (Trel_conj (red1 Σ (Γ ,,, fix_context mfix0))
          (P Σ (Γ ,,, fix_context mfix0))) dbody
            (fun x => (dname x, dtype x, rarg x))) mfix0 mfix1 ->
          P Σ Γ (tFix mfix0 idx) (tFix mfix1 idx)) ->

       (forall (Γ : context) (mfix0 mfix1 : list (def term)) (idx : nat),
        OnOne2 (on_Trel_eq (Trel_conj (red1 Σ Γ) (P Σ Γ)) dtype (fun x => (dname x, dbody x, rarg x))) mfix0 mfix1 ->
        P Σ Γ (tCoFix mfix0 idx) (tCoFix mfix1 idx)) ->
       ...
       forall (Γ : context) (t t0 : term), red1 Σ Γ t t0 -> P Σ Γ t t0.
Proof.
